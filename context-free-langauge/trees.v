...
simpl in H2.
assert (H3: (bheight bt1) >= (bheight bt2) \/ (bheight bt1) <= (bheight bt2)) by lia.
...
- simpl.
  rewrite app_length.
  assert (H: bheight t1 >= bheight t2 \/ bheight t1 <= bheight t2) by lia.
  destruct H as [H | H].
  + apply max_l in H.
    rewrite H.
...
- simpl.
  ...
  + assert (H':= H).
    ...
    assert (H2: length (bfrontier t1) <= 2 ^ (bheight t2 - 1)) by lia.
    assert (H3: length (bfrontier t1) + length (bfrontier t2) <= 2 ^ (bheight t2 - 1) + 2 ^ (bheight t2 - 1)) by lia.
  ...
  + assert (H':= H).
    ...
    assert (H2: length (bfrontier t2) <= 2 ^ (bheight t1 - 1)) by lia.
    assert (H3: length (bfrontier t1) + length (bfrontier t2) <= 2 ^ (bheight t1 - 1) + 2 ^ (bheight t1 - 1)) by lia.
...
apply contrap in H2.
- assert (H3: bheight t < i \/ bheight t > i \/ bheight t = i) by lia.
...
- destruct IHt1 as [p1 [H1]].
  ...
  assert (H3: bheight t1 >= bheight t2 \/ bheight t1 <= bheight t2) by lia.
  destruct H3 as [H3 | H3].
  + exists ((inl n) :: p1).
    split.
    * {
...
- split.
  ...
  + assert (H3':= H3).
    ...
    * rewrite H3 in H4.
      ...
      simpl in H4.
      assert (H5: length p' = z + (length ntl + 1)) by lia.
      clear H4.
...
- intros p H.
  ...
  + inversion H0.
    ...
    simpl.
    specialize (IHt1 p1 H1).
    assert (H2: bheight bt1 >= bheight bt2 \/ bheight bt1 <= bheight bt2) by lia.
  ...
  + clear H IHbt2.
    ...
    * inversion H0.
      ...
      - simpl.
        reflexivity.
      - simpl.
        assert (H4: bheight bt0 >= bheight bt3 \/ bheight bt0 <= bheight bt3) by lia.
...
assert (H5: bheight t1 >= bheight t2 \/ bheight t1 <= bheight t2) by lia.
...
- rewrite H2 in H1.
  ...
  + apply bheight_left in H1.
    * rewrite H1, H4.
      lia.
    * exact H5.
- assert (H6: bheight t1 < bheight t2 \/ bheight t1 = bheight t2) by lia.
  ...
  + clear H5.
    ...
    assert (H7: (max (bheight t1) (bheight t2) = bheight t2)).
      {
      assert (H7: bheight t1 <= bheight t2) by lia.
...
assert (H5: bheight t1 >= bheight t2 \/ bheight t1 <= bheight t2) by lia.
destruct H5 as [H5 | H5].
- assert (H6: bheight t2 < bheight t1 \/ bheight t2 = bheight t1) by lia.
  destruct H6 as [H6 | H6].
  + clear H5.
    rewrite H2 in H1.
    simpl in H1.
...
- intros p1 p2 c H1 H2 H3 HH.
  ...
  + clear H1.
    ...
    * inversion H.
      clear H.
      subst.
      assert (H6: length p1 = 0 \/ length p1 > 0) by lia.
      {
      destruct H6 as [H6 | H6].
