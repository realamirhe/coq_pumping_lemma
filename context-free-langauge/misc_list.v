...
- remember (a::l) as l0.
  ...
  + rewrite app_length.
    ...
    rewrite last_last.
    rewrite app_nth2.
    * assert (H: (Datatypes.length (removelast l0) + 1 - 1 - Datatypes.length (removelast l0) = 0)) by lia.
      rewrite H.
      simpl.
      reflexivity.
    * lia.
...
Proof.
intros A def l H1.
assert (H2: length l > 2 \/ length l = 2) by lia.
...
rewrite H5 in H1.
assert (H6: length (skipn n l) = m) by lia.
