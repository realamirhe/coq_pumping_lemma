...
- intros a0.
  ...
  + simpl.
    assert (H2: (length_max_sf s) <= (length l) \/
                (length_max_sf s) > (length l)) by lia.
    ...
    * apply leb_correct_conv in H2.
      ...
      assert (H3: (length_max_sf s) <= (length a0) \/
                  (length_max_sf s) > (length a0)) by lia.
      {
      destruct H3 as [H3 | H3].
      - apply leb_correct in H3.
        rewrite H3.
...
- intros n a H2.
  ...
  + apply leb_correct_conv in H3.
    ...
    assert (H4: (length_max_sf s) <= (length a) \/
                (length_max_sf s) > (length a)) by lia.
    destruct H4 as [H4 | H4].
    * apply leb_correct in H4.
      rewrite H4 in H2.
      {
...
- intros l2 n H1 H2.
  ...
  assert (H5: length_max_sf (l1 ++ l2) > length a \/
              length_max_sf (l1 ++ l2) <= length a) by lia.
...
- intros left right H1.
  ...
  + subst.
    ...
    assert (H2: (length_max rl) <= (length right) \/
                (length_max rl) > (length right)) by lia.
    destruct H2 as [H2 | H2].
    * apply leb_correct in H2.
      rewrite H2.
      lia.
...
- simpl in H2.
  assert (H3: (length_max_sf (concat_symbol a l)) <= (S (length a0)) \/
              (length_max_sf (concat_symbol a l)) > (S (length a0))) by lia.
  destruct H3 as [H3 | H3].
  + apply leb_correct in H3.
    rewrite H3 in H2.
    lia.
...
- intros n a0 H.
  ...
  assert (H4: (length_max (all_rules_match a0 s)) <= (length a) \/
              (length_max (all_rules_match a0 s)) > (length a)) by lia.
  destruct H4 as [H4 | H4].
  + apply leb_correct in H4.
    rewrite H4.
    exact H10.
...
- intros nl v l r H0 H1 H2.
  simpl.
  apply in_or_app.
  assert (H3: length r = S n \/ length r <= n) by lia.
  clear H2.
  destruct H3 as [H3 | H3].
  + left.
    apply all_rules_with_correct.
